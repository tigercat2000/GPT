...
/datum/tgui_panel/proc/analyze_telemetry(payload)
	if(world.time > telemetry_requested_at + TGUI_TELEMETRY_RESPONSE_WINDOW)
		message_admins("[client] sent telemetry outside of the allocated time window.")
		return
	if(telemetry_analyzed_at)
		message_admins("[client] sent telemetry more than once.")
		return
	...
	if(len > TGUI_TELEMETRY_MAX_CONNECTIONS)
		message_admins("[client] was kicked for sending a huge telemetry payload")
		qdel(client)
		return
	var/list/found
	...
	if(found)
		var/msg = "[client] has a banned account in connection history! (Matched: [found["ckey"]], [found["address"]], [found["computer_id"]])"
		message_admins(msg)
