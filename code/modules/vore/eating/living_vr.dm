...
/mob/living/proc/vore_attackby(obj/item/I,mob/user)
	...
	if(istype(I,/obj/item/weapon/grab))
		...
		if((G.state >= GRAB_AGGRESSIVE) && (isliving(user) && isliving(G.affecting)))
			...
			else if((src == G.affecting) && (is_vore_predator(G.affecting)))
...
//
/mob/living/proc/handle_internal_contents(seconds, times_fired)
	if(times_fired % 3 != 1)
	...
	/*if(noisy > 0 && nutrition < 100 && prob(10))
		var/growlsound = pick(hunger_sounds) // TODO
		if(nutrition < 50)
	...
	if(times_fired % 90 != 1)
		return //Occasionally do supercleanups.
	for(var/I in vore_organs)
		var/datum/belly/B = vore_organs[I]
		if(B.internal_contents.len)
