...
/datum/belly/proc/release_all_contents()
	...
	for(var/atom/movable/AM in internal_contents)
		if(isliving(AM))
			var/mob/living/L = AM
			if(L.absorbed)
				continue
...
/datum/belly/proc/release_specific_contents(var/atom/movable/AM)
	if(!(AM in internal_contents))
		return 0 // They weren't in this belly anyway

	AM.forceMove(owner.loc)  // Move the belly contents into the same location as belly's owner.
	internal_contents -= AM  // Remove from the belly contents
	if(AM in items_preserved)
		items_preserved -= AM
	...
	owner.visible_message("<font color='green'><b>[owner] expels [AM] from their [lowertext(name)]!</b></font>")
	owner.update_icons()
	return 1
...
/datum/belly/proc/relay_resist(var/mob/living/R)
	...
	for(var/mob/M in hearers(4, owner))
		to_chat(M, struggle_outer_message)
	R << struggle_user_message

	/*var/strpick = pick(struggle_sounds) // TODO
