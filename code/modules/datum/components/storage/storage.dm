...
/datum/component/storage/proc/attack_self(datum/source, mob/M)
	...
	if(locked)
		...
		return FALSE
	if((M.get_active_hand() == parent) && allow_quick_empty)
...
/datum/component/storage/proc/quick_empty(mob/M)
	var/atom/A = parent
	if(A.loc != M || M.stat != CONSCIOUS || M.restrained() || !M.canmove)
		return
	if(locked)
		to_chat(M, "<span class='warning'>[parent] seems to be locked!</span>")
		return FALSE
	...
	if(ismob(M)) //all the check for item manipulation are in other places, you can safely open any storages as anything and its not buggy, i checked
		...
		if(!M.incapacitated())
			...
			if(A.loc != M)
				return
			playsound(A, "rustle", 50, 1, -5)
			if(istype(over_object, /obj/screen/inventory/hand)) // TODO
				var/obj/screen/inventory/hand/H = over_object
				if(!M.unEquip(A))
					to_chat(M, "<span class='warning'>[A] is stuck to you!</span>")
					return
				M.equip_to_slot(A, H.slot_id)
				return
			//A.add_fingerprint(M)
...
/datum/component/storage/proc/mob_item_insertion_feedback(mob/user, mob/M, obj/item/I, override = FALSE)
	...
	for(var/mob/viewing in viewers(user, null))
		if(M == viewing)
			...
			to_chat(viewing, "<span class='notice'>[M] puts [I] [insert_preposition]to [parent].</span>")
...
/datum/component/storage/proc/show_to_ghost(datum/source, mob/ghost/M)
...
/datum/component/storage/proc/check_locked(datum/source)
	return locked
...
/datum/component/storage/proc/on_attack_hand(datum/source, mob/user)
	...
	// 	if(H.l_store == A && !H.get_active_hand())	//Prevents opening if it's in a pocket.
	// 		. = COMPONENT_NO_ATTACK_HAND
	// 		H.put_in_hands(A)
	// 		H.l_store = null
	// 		return
	// 	if(H.r_store == A && !H.get_active_hand())
	// 		. = COMPONENT_NO_ATTACK_HAND
	// 		H.put_in_hands(A)
	// 		H.r_store = null
	// 		return
...
/datum/component/storage/proc/gather_mode_switch(datum/source, mob/user)
